...
struct GetIndustryImage : Codable {
    let id : Int?
    let url_lg : String?
    let url_md : String?
    let url_sm : String?
...
struct GetIndustryData : Codable {
    let id : Int?
    let title : String?
    let phone : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        image = try values.decodeIfPresent(GetIndustryImage.self, forKey: .image)
    }
    
}

import Foundation
struct IndustryLink : Codable {
    let first : String?
    let last : String?
    let prev : String?
    let next : String?
...
import Foundation
struct GetIndustryModel : Codable {
    let industryData : [GetIndustryData]?
    let industryLink : IndustryLink?
    let industryMeta : IndustryMeta?
    
    enum CodingKeys: String, CodingKey {
        
        case industryData = "data"
        case industryLink = "link"
        case industryMeta = "meta"
    }
    
    init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        industryData = try values.decodeIfPresent([GetIndustryData].self, forKey: .industryData)
        industryLink = try values.decodeIfPresent(IndustryLink.self, forKey: .industryLink)
        industryMeta = try values.decodeIfPresent(IndustryMeta.self, forKey: .industryMeta)
    }
}
