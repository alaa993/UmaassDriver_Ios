...
class MessageViewController: UIViewController {
    ...
    func getData()  {
        ...
        ServiceAPI.shared.fetchGenericData(urlString: baseUrl+"notifications?\(languge)",parameters: [:], methodInput: .get, isHeaders: true) {(model:ModelNotificationMessage?, error:Error?,status:Int?) in
            
            self.dismissLoding()
            self.model = model?.data?.filter{$0.app == "Provider"} ?? []
